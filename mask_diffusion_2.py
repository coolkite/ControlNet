...
from cldm.ddim_hacked_mask import DDIMSampler_Mask
...
model_nomal = model_normal.cuda()
model_masked = model_masked.cuda()

ddim_sampler_normal = DDIMSampler(model_normal)
ddim_sampler_masked = DDIMSampler_Mask(model_masked)
...
def log_run(input_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, low_threshold, high_threshold, output_images):
    ...
    log_dir = "run_logs"
    os.makedirs(log_dir, exist_ok=True)
    log_file = os.path.join(log_dir, f"2_masked_diffusion_compare.html")
...
def process(input_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, ddim_steps, guess_mode, strengths, scale, seed, eta, low_threshold, high_threshold):
    ...
    for strength in [strengths, strengths*1.5, strengths*2]:
        with torch.no_grad():
            ...
            cond_mask = {"c_concat": [control], "c_crossattn": [model_normal.get_learned_conditioning([prompt + ', ' + a_prompt] * num_samples)]}
            un_cond_mask = {"c_concat": None if guess_mode else [control], "c_crossattn": [model_normal.get_learned_conditioning([n_prompt] * num_samples)]}
            shape = (4, H // 8, W // 8)
        ...
        output_images = [detected_map] + n_result + m_result + [mask]
        ...
        #create_output_video(model, intermediates, results, ddim_steps, mask, prompt.replace(" ", "_")+str(seed))
