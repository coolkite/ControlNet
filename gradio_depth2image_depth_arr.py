...
def process(prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta):
    with torch.no_grad():
        #input_image = HWC3(input_image)
        data = np.load("/project/pi_ekalogerakis_umass_edu/dshivashok/ControlNet/depth_maps/model_depth_views (2).npz")["arr_0"][0]
...
with block:
    ...
    ips = [prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta]
