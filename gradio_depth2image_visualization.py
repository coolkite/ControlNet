...
model = create_model('./models/cldm_v15_visualization.yaml').cpu()
model.load_state_dict(load_state_dict('./models/control_sd15_depth.pth', location='cuda'))
model = model.cuda()
ddim_sampler = DDIMSampler(model)


def process(input_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, interval=1):
    with torch.no_grad():
...
def create_output_video(model, intermediates, results, ddim_steps, mask, name, interval=1):
    ...
    print(f"{name}.mp4")
    video_path = os.path.join(log_dir, f"{name}_depth_control_interval_{interval}.mp4")
...
with block:
    ...
    ips = [input_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, interval]
