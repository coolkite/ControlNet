...
def log_run(input_image, prompt, bg_prompt, a_prompt, n_prompt, num_samples, image_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, low_threshold, high_threshold, output_images):
    ...
    log_file = os.path.join(log_dir, f"masked_diffusion.html")
...
with block:
    ...
    with gr.Row():
        with gr.Column():
            ...
            prompt = gr.Textbox(label="Prompt", value="mahogany chair")
            bg_prompt = gr.Textbox(label="Background Prompt", value="detailed, well-light living room")
