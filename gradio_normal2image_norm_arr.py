...
def process(prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, bg_threshold, idx=0):
    with torch.no_grad():
        #input_image = HWC3(input_image)
        data = np.load("/project/pi_ekalogerakis_umass_edu/dshivashok/ControlNet/maps/normal_maps/model_normal_maps_views.npz")["arr_0"][idx]
        data = data.astype(np.float32)
        print(data.dtype, data.shape)
        #_, detected_map = apply_midas(resize_image(input_image, detect_resolution), bg_th=bg_threshold)

        detected_map = HWC3(data)
        print(data.shape)
...
def create_output_video(model, intermediates, results, ddim_steps, mask, name):
    ...
    log_dir = "sample_run_meeting/normal_map"
    os.makedirs(log_dir, exist_ok=True)
    print(f"{name}.mp4")
    video_path = os.path.join(log_dir, f"{name}_normal.mp4")
...
with block:
    ...
    with gr.Row():
        with gr.Column():
            ...
            prompt = gr.Textbox(label="Prompt", value="a photo of a chair")
            run_button = gr.Button(label="Run")
            with gr.Accordion("Advanced options", open=False):
                idx = gr.Slider(label="Images", minimum=0, maximum=19, value=1, step=1)
    ...
    ips = [prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, bg_threshold, idx]
