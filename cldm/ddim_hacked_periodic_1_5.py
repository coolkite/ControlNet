...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape,
        ...
        a_cond = {}
        a_cond["c_concat"] = cond["c_concat"]
        if "learned_embedding" in cond:
            a_cond["c_crossattn"] = [self.second_model_sd.get_learned_conditioning(cond["new_prompt"], learned_embedding=cond["learned_embedding"])]
        else:
            a_cond["c_crossattn"] = [self.second_model_sd.get_learned_conditioning(cond["new_prompt"])]
        
        a_uncond = {}
        a_uncond["c_concat"] = unconditional_conditioning["c_concat"]
        a_uncond["c_crossattn"] = [self.second_model_sd.get_learned_conditioning(unconditional_conditioning["new_prompt"])]
        ...
        for i, step in enumerate(iterator):
            ...
            if i <= interval:
                ...
                new_cond = {}
                new_cond["c_concat"] = a_cond["c_concat"] #cond["c_concat"]
                #new_cond = cond["c_crossattn_no_ctrl"]
                if "learned_embedding" in cond:
                    new_cond["c_crossattn"] = a_cond["c_crossattn"] #[self.second_model_sd.get_learned_conditioning(cond["new_prompt"], learned_embedding=cond["learned_embedding"])]
                else:
                    new_cond["c_crossattn"] = a_cond["c_crossattn"] #[self.second_model_sd.get_learned_conditioning(cond["new_prompt"])]
                
                new_uncond = {}
                new_uncond["c_concat"] = a_uncond["c_concat"] #unconditional_conditioning["c_concat"]
                new_uncond["c_crossattn"] = a_uncond["c_crossattn"] #[self.second_model_sd.get_learned_conditioning(unconditional_conditioning["new_prompt"])]

                print(new_cond)
                print(new_uncond)

                outs = self.p_sample_ddim_1_5(img, new_cond, ts, index=index, use_original_steps=ddim_use_original_steps,
                                      quantize_denoised=quantize_denoised, temperature=temperature,
                                      noise_dropout=noise_dropout, score_corrector=score_corrector,
                                      corrector_kwargs=corrector_kwargs,
                                      unconditional_guidance_scale=unconditional_guidance_scale,
    ...
    def p_sample_ddim_cldm(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            model_output = self.model.apply_model(x, t, c, **kwargs) #img, t, cond
        else:
    ...
    def p_sample_ddim_1_5(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
                      unconditional_guidance_scale=1., unconditional_conditioning=None,
                      dynamic_threshold=None,
                      **kwargs
