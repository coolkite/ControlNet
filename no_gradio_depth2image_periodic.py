...
def process(input_image_path, prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, interval=1):
    ...
    with torch.no_grad():
        ...
        cond = {"c_concat": [control], "c_crossattn": [model.get_learned_conditioning([prompt + ', ' + a_prompt] * num_samples)], "c_crossattn_chair": [model.get_learned_conditioning([prompt + ', ' + a_prompt] * num_samples)], "c_crossattn_other": [model.get_learned_conditioning(["a photo of a chair with slat back" + ', ' + a_prompt] * num_samples)]}
        ...
        create_output_video(model, intermediates, results, ddim_steps, detected_map, f"{prompt}_{seed}", interval)
...
def create_output_video(model, intermediates, results, ddim_steps, mask, name, interval=1):
    ...
    log_dir = f"partial_complete_until_20/a_photo_of_a_chair_with_slat_back/control_before_{interval}"
    os.makedirs(log_dir, exist_ok=True)
    video_path = os.path.join(log_dir, f"{name}_depth_{interval}.mp4") #f"{name}_depth_control_before_{interval}.mp4"
