...
import re
import sys
sys.path.append("/project/pi_ekalogerakis_umass_edu/dshivashok/ControlNet/evaluations/dino-vit-features")
...
sys.path.append("/project/pi_ekalogerakis_umass_edu/dshivashok/ControlNet/evaluations/segment-anything")
...
def process_image_pair(original_depth_path, compared_depth_path, compared_img_path, interval_path, output_suffix, mask, output_dir):
    ...
    original_depth = generate_mask_from_original_depth(original_depth_path)
...
def generate_mask_from_original_depth(original_depth_path):
    """Generate mask from original depth image and convert white background to black."""
    original_depth = load_image(original_depth_path, grayscale=True)
    
    # Threshold the image to create a binary mask
    # Assuming white is 255 and the object is darker
    _, mask = cv2.threshold(original_depth, 254, 255, cv2.THRESH_BINARY_INV)
    
    # Create a black background image
    black_background = np.zeros_like(original_depth)
    
    # Apply the mask to keep only the object
    masked_depth = cv2.bitwise_and(original_depth, original_depth, mask=mask)
    
    # Combine the masked depth with the black background
    result = cv2.add(masked_depth, black_background)
